...
$student_name = $admission_no = $term = $a_session = $student_class = $class_arm =  $subject = $T1 = $T2 = $project = $assignment = $exam = $total_score =  "";
...
if (isset($_POST['save_exam_scores'])) {
    ...
    for ($i = 0; $i < $total_t1; $i++) {
        ...
        if (!empty($row) && $row['admission_no'] === $admission_no) {
            $errors = "Scores for " . $subject . "  for " . $student_name . " with " . $admission_no . " already exist, please check your parameters and try again!  ";
